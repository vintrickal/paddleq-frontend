...
/// Reusable "Welcome" splash animation — the rings, dashed net, paddle /
/// ball / scribble confetti, rising `PADDLE` wordmark with a hopping
/// ball-dot, eyebrow tagline, script sub-line and pulse bar — extracted
/// from `WelcomePage` so other screens (notably the Player Profile loading
/// gate) can reuse the same intro.
///
/// Sequence (animation peaks at [animationMs], holds until [totalMs] so
/// the user sees the finished composition, then the [exitMs] fade fires
...
///   • [totalMs] — page fades / scales out
///   • [totalMs] + [exitMs] — [onComplete] fires
///
/// Pass [onSkip] to control what happens when the user taps the floating
/// SKIP button (default: same as [onComplete], after the exit fade). Pass
/// `showSkip: false` to hide the button entirely — useful when this is a
/// loading gate rather than a true splash.
class WelcomeAnimationView extends StatefulWidget {
  const WelcomeAnimationView({
    super.key,
    required this.onComplete,
    this.onSkip,
    this.showSkip = true,
    this.animationMs = 3700,
    this.totalMs = 5500,
    this.exitMs = 480,
  });

  /// Fires after the exit fade finishes. The caller decides what happens
  /// next (route away, swap in real content, etc.).
  final VoidCallback onComplete;

  /// Optional: if provided, replaces the default SKIP behavior. Useful if
  /// you want skipping to fire `onComplete` immediately (no exit fade) or
  /// to navigate somewhere different.
  final VoidCallback? onSkip;

  /// Hide the SKIP button entirely (e.g. when this view is a loading gate
  /// and the user has nothing to skip *to*).
  final bool showSkip;

  /// Time the wordmark, sub and pulse bar take to reach their final state.
  final int animationMs;

  /// Total time the screen stays on-screen before exiting.
  /// `[animationMs]..[totalMs]` is a hold window so the user gets to see
  /// the finished composition (and the ball-hop loop) in full.
  final int totalMs;

  /// Duration of the exit fade / scale-out.
  final int exitMs;

  @override
  State<WelcomeAnimationView> createState() => _WelcomeAnimationViewState();
}

class _WelcomeAnimationViewState extends State<WelcomeAnimationView>
  ...
  void initState() {
    ...
    Future.delayed(
      Duration(milliseconds: widget.totalMs),
      _exitAndComplete,
    );
  }
  ...
  Future<void> _exitAndComplete() async {
    if (_completed) return;
    _completed = true;
    if (!mounted) return;
    setState(() => _exiting = true);
    await Future.delayed(Duration(milliseconds: widget.exitMs));
    if (!mounted) return;
    widget.onComplete();
  }
        ...
        builder: (context, _) {
          return Stack(
            ...
            children: [
              ...
              _RingsLayer(progressMs: _master.value * widget.animationMs),
              _NetLayer(progressMs: _master.value * widget.animationMs),
              _ConfettiLayer(
                fliers: _fliers,
                progressMs: _master.value * widget.animationMs,
              ),
              Center(
                child: AnimatedScale(
                  scale: _exiting ? 1.04 : 1,
                  duration: Duration(milliseconds: widget.exitMs),
              ...
              if (widget.showSkip)
                Positioned(
                  ...
                  child: AnimatedOpacity(
                    opacity: _exiting
                        ? 0
                        : _fade(_master.value * widget.animationMs, 800, 1200),
